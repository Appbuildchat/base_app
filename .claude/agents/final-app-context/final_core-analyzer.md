...
1. **Generate Core File Lists**: Run core_list_generator.py to create category-separated file lists
2. **Dynamic Category Processing**: Process each category from output/core-list/ directory
...
5. **Save Output**: Save the final report as `output-final-app-context/1.core-analysis.md`

## Dynamic Processing Commands

**Step 1: Generate Core File Lists**
```bash
cd /home/test1/aas/AAS_0.2.0/flutter-base-project
python3 .claude/agents/final-app-context/final-core_list_generator.py
```
...
for category_file in output/final-core-list/*-core-files.md; do
...
1. **Dynamic Processing**: MUST process ALL categories from output/final-core-list/ directory sequentially
...
## Success Criteria
- ALL categories from output/final-core-list/ must be processed
...
All categories are automatically discovered from `output/final-core-list/` directory. Categories typically include:
...
- Any files not captured by final-core_list_generator.py patterns

When complete, save your analysis to `output-final-app-context/1.core-analysis.md` and respond with:
"Core architecture analysis completed and saved to output-final-app-context/1.core-analysis.md"