...
from .modules.provider_config import ProviderConfig

app = typer.Typer()
console = Console()

def check_provider_setup(provider: str, model: str):
    ...
    if not is_valid:
        ...
        # Provide helpful setup instructions based on provider
        if provider == "openrouter":
            console.print("Please set it with: export OPENROUTER_API_KEY=your-api-key")
            ...
            console.print("Please make sure Ollama is running: ollama serve")

        sys.exit(1)

def check_api_key():
    """Check if OpenAI API key is set (for backward compatibility)."""
    if not os.getenv("OPENAI_API_KEY"):
        console.print(f"[red]Error: {ERROR_NO_API_KEY.format('OPENAI_API_KEY')}[/red]")
        console.print("Please set it with: export OPENAI_API_KEY=your-api-key")
        sys.exit(1)
...
def run(
    ...
    check_provider_setup(provider, model)
